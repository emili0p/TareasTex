...
\subsection{Nota sobre Modelado y Motor de Base de Datos}

El modelo lógico de la base de datos fue representado mediante herramientas de diagramación compatibles con SQL Server únicamente con fines de modelado visual y estructuración conceptual.

La implementación real del sistema se realizará sobre \textbf{MariaDB}, utilizando el ORM \textbf{Eloquent} provisto por Laravel. 

El diseño propuesto es compatible con cualquier sistema gestor de bases de datos relacional que soporte:

\begin{itemize}
    \item Integridad referencial mediante llaves foráneas.
    \item Restricciones \texttt{UNIQUE} y \texttt{CHECK}.
    \item Índices en campos de alta frecuencia de consulta.
    \item Transacciones ACID.
\end{itemize}

El modelo se encuentra normalizado hasta \textbf{Tercera Forma Normal (3FN)}, garantizando:

\begin{itemize}
    \item Eliminación de redundancia innecesaria.
    \item Separación adecuada de responsabilidades entre entidades.
    \item Consistencia estructural y escalabilidad futura.
\end{itemize}

Se adopta una arquitectura de tipo \textbf{Monolito Modular}, donde el backend se organiza por dominios funcionales alineados al modelo académico definido: 

\begin{itemize}
    \item Gestión de Usuarios y Roles
    \item Gestión Académica (Carreras, Materias, Grupos, Periodos)
    \item Inscripciones y Calificaciones
    \item Tutorías
    \item Créditos Complementarios
    \item Bitácora y Auditoría
\end{itemize}
...
\end{itemize}
\section{Alineación con el Modelo Académico}

La arquitectura técnica propuesta se encuentra alineada con el modelo lógico previamente definido, el cual contempla:

\begin{itemize}
    \item 14 entidades base del sistema académico.
    \item 2 entidades adicionales para la gestión de Créditos Complementarios.
    \item Relación académica principal: Carrera $\rightarrow$ Materia $\rightarrow$ Grupo $\rightarrow$ Inscripción $\rightarrow$ Calificación.
    \item Tutoría como componente complementario vinculado a Alumno, Docente y Periodo.
    \item Bitácora transversal para auditoría de acciones del sistema.
\end{itemize}

Esta alineación garantiza coherencia entre el diseño funcional, el modelo de datos y la implementación técnica.
